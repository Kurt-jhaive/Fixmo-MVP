// Profile Manager for Service Provider Dashboard
class ProfileManager {
    constructor() {
        ...
        this.token = localStorage.getItem('fixmo_provider_token'); // Get token from localStorage
        this.init();
    }

    async init() {
        console.log('ProfileManager init called');
        // Try to get provider data from the main dashboard first
        if (window.dashboard && window.dashboard.providerData) {
            this.currentProvider = window.dashboard.providerData;
            console.log('Using provider data from dashboard:', this.currentProvider);
        } else if (!this.currentProvider) {
            console.log('No provider data available, attempting to fetch...');
            await this.loadProviderData();
        } else {
            console.log('Using existing provider data:', this.currentProvider);
        }
    ...
    async loadProviderData() {
        try {
            ...
            if (response.ok) {
                ...
                console.log('Provider data loaded:', this.currentProvider);
            } else {
    ...
    setupEventListeners() {
        console.log('Setting up ProfileManager event listeners');
        ...
        if (!this.eventListenerAdded) {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (e.target.id === 'profilePhotoInput') {
                    // Profile photo changing is disabled
    ...
    renderStaticProfile() {
        ...
        console.log('Rendering profile with data:', this.currentProvider);

        const profilePhotoUrl = this.currentProvider.provider_profile_photo 
            ? `/${this.currentProvider.provider_profile_photo}`
            ...
            <div class="modern-profile-container">
                ...
                <div class="profile-header">
                    ...
                    <div class="profile-info-header">
                        <h2>${this.currentProvider.provider_first_name} ${this.currentProvider.provider_last_name}</h2>
                        <p class="provider-email">${this.currentProvider.provider_email}</p>
                        <div class="verification-status">
                            <i class="fas fa-shield-alt ${this.currentProvider.provider_isVerified ? 'verified' : 'unverified'}"></i>
                            <span>${this.currentProvider.provider_isVerified ? 'Verified Provider' : 'Pending Verification'}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="profile-card">
                        ...
                        <div class="card-content">
                            <div class="field-group">
                                ...
                                <div class="field-value">${this.currentProvider.provider_first_name}</div>
                            </div>
                            <div class="field-group">
                                <label>Last Name</label>
                                <div class="field-value">${this.currentProvider.provider_last_name}</div>
                            </div>
                            <div class="field-group">
                                <label>Birthday</label>
                                <div class="field-value">${this.currentProvider.provider_birthday ? new Date(this.currentProvider.provider_birthday).toLocaleDateString() : 'Not provided'}</div>
                            </div>
                            <div class="field-group">
                                <label>Location/Address</label>
                                <div class="field-value">${this.currentProvider.provider_location || 'Not provided'}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="profile-card">
                        ...
                        <div class="card-content" id="editableContent">
                            <div class="field-group">
                                ...
                                <div class="field-value" id="emailDisplay">${this.currentProvider.provider_email}</div>
                                <input type="email" class="field-input" id="emailInput" value="${this.currentProvider.provider_email}" style="display: none;">
                            </div>
                            <div class="field-group">
                                <label>Phone Number</label>
                                <div class="field-value" id="phoneDisplay">${this.currentProvider.provider_phone_number || 'Not provided'}</div>
                                <input type="" class="field-input" id="phoneInput" value="${this.currentProvider.provider_phone_number || ''}" style="display: none;">
                            </div>
                        </div>
    ...
    cancelEdit() {
        ...
        // Reset values
        emailInput.value = this.currentProvider.provider_email;
        phoneInput.value = this.currentProvider.provider_phone_number || '';
    ...
    async handleProfileUpdate() {
        ...
        const emailChanged = newEmail !== this.currentProvider.provider_email;
        const phoneChanged = newPhone !== this.currentProvider.provider_phone_number;
        ...
        if (emailChanged) {
            await this.requestOtp(this.currentProvider.provider_email, 'verify_original_email');
        } else {
            await this.requestOtp(this.currentProvider.provider_email, 'verify_phone_only');
        }
    }

    async requestOtp(email, step = 'verify_phone_only') {
        try {
            const response = await fetch('/api/serviceProvider/profile-update-request-otp', {
                method: 'POST',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json',
    ...
    showOtpModal(step = 'verify_phone_only') {
        ...
        if (step === 'verify_original_email') {
            message = `We've sent a 6-digit verification code to your current email address (${this.currentProvider.provider_email}). Please enter it below to proceed with the email change.`;
            ...
            message = `We've sent a 6-digit verification code to your email address (${this.currentProvider.provider_email}). Please enter it below to confirm your profile changes.`;
        }
    ...
    async verifyOtp() {
        ...
        try {
            ...
            if (this.currentOtpStep === 'verify_original_email') {
                endpoint = '/api/serviceProvider/profile-update-verify-original-email';
                requestBody = {
                    provider_email: this.currentProvider.provider_email,
                    provider_phone_number: this.pendingChanges.phone,
                    new_email: this.pendingChanges.email,
                    otp: otp
                };
            } else if (this.currentOtpStep === 'verify_new_email') {
                endpoint = '/api/serviceProvider/profile-update-verify-new-email';
                requestBody = {
                    new_email: this.pendingChanges.email,
                    otp: otp
                };
                ...
                endpoint = '/api/serviceProvider/profile-update-verify-otp';
                const formData = new FormData();
                formData.append('provider_email', this.currentProvider.provider_email);
                formData.append('provider_phone_number', this.pendingChanges.phone);
                ...
                if (response.ok) {
                    ...
                    if (window.dashboard && window.dashboard.updateProfileDisplay) {
                        window.dashboard.providerData = result.provider;
    ...
    async resendOtp() {
        ...
        let emailToSend = this.currentProvider.provider_email;
...
// Initialize Profile Manager when needed
